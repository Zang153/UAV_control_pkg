...
int main(int argc, char **argv)
{
    ...
    Controller_Circle Controller_Circle;
    ...
    while(ros::ok())
    {
        switch (Num_StateMachine)
        {
            ...
            case 0:
                cout << ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>--------<<<<<<<<<<<<<<<<<<<<<<<<<<< "<< endl;
                cout << "Input the flag:  0 for Move_ENU，1 for Move_Body，2 for Land,3 for Disarm ,4 for Hold, 5 for PPN Land, 6 for Idle,7 for Takeoff to default height, 8 for trajectory tracking, 9 for circle"<<endl;
                cin >> flag_1;
        ...
        case 10:
            ...
            while(time_trajectory < trajectory_total_time)
            {
                ...
                Command_Now.Reference_State = Controller_Circle.Circle_trajectory_generation(time_trajectory);
