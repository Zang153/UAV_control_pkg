...
void vector_cb(const px4_command::VectorPayloadtoQuad::ConstPtr& msg)
{
    _VectorPtoQ = *msg;
}
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>主 函 数<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
int main(int argc, char **argv)
{
    ...
    ros::Subscriber vector_sub = nh.subscribe<px4_command::VectorPayloadtoQuad>("/PtoQVector/pose", 10, vector_cb);

    ros::Publisher output_pub = nh.advertise<px4_command::ControlOutput>("/Output", 10);
    // 参数读取
    ...
    pos_controller_N pos_controller_n;
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>主  循  环<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    while(ros::ok())
    {
        ...
        case command_to_mavros::Move_ENU:
            ...
            //cout << "Testing before Vector position!" << _VectorPtoQ.position[0] << _VectorPtoQ.position[1] << _VectorPtoQ.position[2] << endl;
            //cout << "Testing before Vector velocity!" << _VectorPtoQ.velocity[0] << _VectorPtoQ.velocity[1] << _VectorPtoQ.velocity[2] << endl;

            _ControlOutput = pos_controller_n.pos_controller(_DroneState, Command_to_gs.Reference_State, _VectorPtoQ, dt);
            
            //cout << "Testing Vector position!" << _VectorPtoQ.position[0] << _VectorPtoQ.position[1] << _VectorPtoQ.position[2] << endl;
            //cout << "Testing Vector velocity!" << _VectorPtoQ.velocity[0] << _VectorPtoQ.velocity[1] << _VectorPtoQ.velocity[2] << endl;
            ...
            throttle_sp[1] = _ControlOutput.Throttle[1];
            throttle_sp[2] = _ControlOutput.Throttle[2];
            output_pub.publish(_ControlOutput);
            _AttitudeReference = px4_command_utils::ThrottleToAttitude(throttle_sp, Command_to_gs.Reference_State.yaw_ref);
