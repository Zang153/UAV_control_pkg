...
void vision_cb(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
	...
	pos_drone_vio[2] = vision_pose.pose.position.z -0.10;
...
int main(int argc, char **argv)
{
    ...
    // 【订阅】vision估计位置
    ros::Subscriber vision_sub = nh.subscribe<geometry_msgs::PoseStamped>("/vrpn_client_node/quad/pose", 10, vision_cb);
...
void printf_info()
{
    ...
    if(flag_use_laser_or_vicon == 0)
    {
        cout <<">>>>>>>>>>>>>>>>>>>>>>>>Vis/OPT Info [ENU Frame]<<<<<<<<<<<<<<<<<<<<<<<<<" <<endl;
